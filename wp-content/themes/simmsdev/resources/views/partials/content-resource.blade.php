                        ...
                        <div x-data="{ selectedAccordionItem: null }" class="w-full divide-y divide-primary-green overflow-hidden ">
                            @foreach($resourcesSectionOne['resource_slider'] as $index => $resource)
                                ...
                                <div class="divide-y divide-[#00676D]">
                                    <button id="{{ $questionId }}" type="button"
                                        ...
                                        :class="selectedAccordionItem === '{{ $index }}' ? 'text-onSurfaceStrong dark:text-onSurfaceDarkStrong font-bold' : 'text-onSurface dark:text-onSurfaceDark font-medium'"
                                        :aria-expanded="selectedAccordionItem === '{{ $index }}'">
                                        {{ ($i) ? $i . '.'  .  ' ' : '' }}  {!! $resource['rfs1_title'] !!}
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke-width="2" stroke="currentColor" class="size-5 shrink-0 transition" aria-hidden="true"
                                            :class="selectedAccordionItem === '{{ $index }}' ? 'rotate-180' : ''">
                    ...
                    @if($resourcesSectionOne['resource_slider'] )
                        <div x-data="{ selectedAccordionItem: null }" class="w-full divide-y divide-primary-green overflow-hidden ">
                            @foreach($resourcesSectionTwo['resource_slider'] as $index => $resource)
                                ...
                                @endphp
                                <div class="divide-y divide-[#00676D]">
                                    <button id="{{ $questionTwoId }}" type="button"
