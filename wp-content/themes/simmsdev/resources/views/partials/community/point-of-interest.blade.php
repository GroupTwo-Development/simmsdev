                ...
                <div class="md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-3 w-full">
                    @foreach ($communities_items['point_of_interest'] as $interest)
                        <div class="flex flex-col justify-center items-center content-center mb-10">
                            <div class="mb-5">
                                <img src="{{ $interest['poi_icon_image']['sizes']['thumbnail']}}" alt="{{ $interest['poi_icon_image']['alt'] }}" class="bg-cover object-contain w-[60px]">
                            </div>
                            <div>
                                <h4 class="text-center text-primary-green tracking-[0.15em] font-semibold mb-3 uppercase">{{ $interest['pot_title'] }}</h4>
                                <div class="text-sm">
                                    {!! $interest['poi_description'] !!}
                                </div>
                            </div>
                        </div>
                    @endforeach
