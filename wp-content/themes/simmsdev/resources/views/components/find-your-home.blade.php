    ...
    <section class="pt-14 lg:pt-20">
        <div class="container mx-auto">
            <div class="text-center mb-8 lg:mb-12">
                <span class="block font-semibold text-primary-green uppercase tracking-[0.15em] mb-3 text-sm lg:text-2xl">{{ $findYourHome['sub_title'] }}</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            @if ($findYourHome['find_your_home'])
                @foreach ($findYourHome['find_your_home'] as $home )
                    <div class="group relative bg-cover bg-center h-96 cursor-pointer" style="background-image: url('{{ wp_get_attachment_image_url($home['image'], 'full') }}');">
                        ...
                        <div class="absolute inset-0 flex flex-col justify-center items-center text-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            ...
                            <x-cta-link href="{{ $home['cta_link']['url'] }}" class="text-white border-b-2 border-white hover:border-simms-gold hover:text-simms-gold transition-all ease-in-out">{{ __('Learn More') }}</x-cta-link>
                        </div>
                    </div>
                @endforeach
            @endif
